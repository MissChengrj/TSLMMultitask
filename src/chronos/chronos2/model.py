...
class Chronos2Model(PreTrainedModel):
    ...
    def _prepare_patched_context(
        ...
        # 基础 attention_mask: 至少有一个有效数据的 patch 为 True
        attention_mask = patched_mask.sum(dim=-1) > 0  # (batch_size, num_patches)
        num_context_patches = attention_mask.shape[-1]

        # ========================================================================= 
        # [核心修复]: 区分 训练(随机掩码) 与 推理(定向插值) 
        # ========================================================================= 
        if self.training: 
            # 训练模式：执行 15% 随机掩码 
            mlm_prob = 0.15 
            # 仅对原本包含真实数据的 patch 进行随机 mask 
            mlm_mask = (torch.rand(patched_context.shape[:-1], device=self.device) < mlm_prob) & attention_mask 
        else: 
            # 推理/插值模式：不再随机！寻找需要修复的 Patch 
            # 如果一个 Patch 中有效数据的数量小于 patch_size，说明它包含 NaN，需要被重构 
            mlm_mask = patched_mask.sum(dim=-1) < self.chronos_config.input_patch_size 
             
            # 【关键修复】：强制唤醒！ 
            # 即使某个 Patch 全是 NaN (原本 attention_mask 为 False)， 
            # 只要我们想修复它，就必须强制它的 attention_mask = True，让其参与双向注意力 
            attention_mask = attention_mask | mlm_mask 

        # 将目标 Patch 的数据清零，充当 [MASK] 表征 
        patched_context = torch.where(mlm_mask.unsqueeze(-1), torch.zeros_like(patched_context), patched_context)
        # 注意：不要用 0 覆盖 patched_mask，保持原始真实分布，方便后续 Loss 计算 
        # ========================================================================= 
    ...
    def _compute_mlm_loss(
        ...
        """专门用于随机掩码的重构损失计算 (MLM Loss)"""
        # 归一化真实的原始 context 作为 Ground Truth 目标
        target, _ = self.instance_norm(original_context, loc_scale)
        target = target.unsqueeze(1).to(self.device)  # (batch, 1, seq_len)

        # 将 Patch 级别的 mlm_mask (batch, num_patches) 展开到时间步级别 (batch, 1, seq_len)
        ...
        # 仅在原本有效 (valid_mask) 且 被随机掩盖 (mlm_mask_expanded) 的位置计算 Loss
        valid_mask = (
        ...
        num_masked = loss_mask.sum(dim=-1).squeeze(1) + 1e-8  # 防止除以 0
        loss = (sum_loss / num_masked).mean()
    ...
    def forward(
        ...
        # ==========================================
        # MLM 模式：提取 context 的隐状态进行自重构
        # ==========================================
        # 使用 context 部分的隐状态（而非 future 部分）进行重构
        ...
        # 计算 MLM 重构损失（无需 future_target，只需 context 即可实现自监督训练）
        loss = (
