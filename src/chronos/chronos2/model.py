...
class Chronos2Model(PreTrainedModel):
    ...
    def _prepare_patched_context(
        ...
        if self.training:
            # 1. 独立随机掩码 
            indep_prob = 0.15
            indep_mask = (torch.rand(patched_context.shape[:-1], device=self.device) < indep_prob)

            # 2. 同步块掩码
            sync_prob = 0.05
            if group_ids is not None:
                unique_groups, group_indices = torch.unique(group_ids, return_inverse=True)
                sync_mask_base = (torch.rand(len(unique_groups), num_context_patches, device=self.device) < sync_prob)
                sync_mask = sync_mask_base[group_indices]
            else:
                sync_mask = torch.zeros_like(indep_mask)

            # 合并并在真实数据范围内应用 MASK
            mlm_mask = (indep_mask | sync_mask) & attention_mask
        else:
            # 推理阶段：利用输入中的 NaN（缺失值）作为插值目标，如果是异常检测则全透明无 Mask
            if is_anomaly_detection:
                mlm_mask = torch.zeros_like(attention_mask)
            else:
                mlm_mask = patched_mask.sum(dim=-1) < self.chronos_config.input_patch_size
                mlm_mask = mlm_mask & attention_mask

        # MASK 区域强制开启注意力，让模型能利用上下文来预测它
        attention_mask = attention_mask | mlm_mask

        # 修复 Data Leakage：同步置零 patched_context 和元特征 patched_mask
        mlm_mask_expanded = mlm_mask.unsqueeze(-1)
        patched_context = torch.where(mlm_mask_expanded, torch.zeros_like(patched_context), patched_context)
        patched_mask = torch.where(mlm_mask_expanded, torch.zeros_like(patched_mask), patched_mask)
    ...
    def _compute_mlm_loss(
        ...
        """稳定的 Masked Time Series Modeling 损失计算"""
        target, _ = self.instance_norm(original_context, loc_scale)
        target = target.unsqueeze(1).to(self.device)  # (batch, 1, seq_len)
        ...
        num_masked = loss_mask.sum(dim=-1).squeeze(1)

        # 安全计算 Masked Loss，避免未被 Mask 的条目导致分母除 0 或梯度爆炸
        safe_num_masked = torch.where(num_masked > 0, num_masked, torch.ones_like(num_masked))
        item_loss = sum_loss / safe_num_masked

        # 仅对存在 Mask 任务的序列进行均值计算
        valid_batch_items = (num_masked > 0).float()
        final_loss = (item_loss * valid_batch_items).sum() / (valid_batch_items.sum() + 1e-8)
    ...
    def encode(
        ...
        # append [REG] special token embedding
        if self.chronos_config.use_reg_token:
        ...
        return encoder_outputs, loc_scale, patched_future_covariates_mask, num_context_patches, mlm_mask
    ...
    def forward(
        ...
        # MTM 解析
        # ==========================================
        reconstruct_embeds = hidden_states[:, :num_context_patches]

        quantile_preds: torch.Tensor = self.output_patch_embedding(reconstruct_embeds)
