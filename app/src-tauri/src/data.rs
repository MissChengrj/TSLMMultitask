...
impl DataProcessor {
    ...
    pub fn run_prediction(
        ...
        let script_path = r"E:\Cursor Code\Chronos\app\engine\bridge.py";
        let model_path = match model_name {
            "Chronos-2" => r"E:\Cursor Code\Chronos\weights\chronos-2",
            "Chronos-T5-Base" => r"E:\Cursor Code\Chronos\weights\mlm_finetuned\mlm-lora-finetuned",
            _ => r"E:\Cursor Code\Chronos\weights\default_model",
        };
    ...
    pub fn get_models(&self) -> Vec<ModelInfo> {
        // ... 保持不变
        vec![
            ModelInfo { name: "Chronos-2".to_string(), description: "Chronos-2 第二代时序大模型".to_string(), supported_tasks: vec!["单变量预测".to_string(), "多变量预测".to_string(), "协变量预测".to_string(), "数据修复".to_string(), "异常检测".to_string()], parameters: vec![ ModelParameter { name: "prediction_length".to_string(), param_type: "integer".to_string(), default: "24".to_string(), description: "预测步数".to_string() }, ModelParameter { name: "context_length".to_string(), param_type: "integer".to_string(), default: "512".to_string(), description: "上下文窗口长度".to_string() }, ModelParameter { name: "anomaly_threshold".to_string(), param_type: "float".to_string(), default: "3.0".to_string(), description: "异常判别基准 (Sigma倍数)".to_string() } ] },
            ModelInfo { name: "Chronos-T5-Base".to_string(), description: "基础模型，平衡性能与速度".to_string(), supported_tasks: vec!["单变量预测".to_string(), "多变量预测".to_string(), "协变量预测".to_string(), "异常检测".to_string()], parameters: vec![ ModelParameter { name: "prediction_length".to_string(), param_type: "integer".to_string(), default: "24".to_string(), description: "预测步数".to_string() }, ModelParameter { name: "anomaly_threshold".to_string(), param_type: "float".to_string(), default: "3.0".to_string(), description: "异常判别基准 (Sigma倍数)".to_string() } ] },
        ]
    }
}